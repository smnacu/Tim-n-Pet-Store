...
def update_stock_producto(db: Session, producto_id: int, cantidad_vendida: int):
    ...
    if db_producto:
        if db_producto.stock < cantidad_vendida:
            raise ValueError(f"Stock insuficiente. Stock actual: {db_producto.stock}, cantidad solicitada: {cantidad_vendida}")
...
def get_productos_by_proveedor(db: Session, proveedor_id: int):
    ...
    """
    return db.query(models.Producto).filter(models.Producto.proveedor_id == proveedor_id).all()
